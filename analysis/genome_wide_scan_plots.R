...
if(!("corrected.p"  %in% names(results))){
    ...
    write.table(results, paste0("~/selection/analysis/",version,"/gscan/scan_results", results.tag, ".txt"),
...
## par(bg = "grey92")
png(paste0("~/selection/analysis/",version,"/gscan/mh_plot", results.tag ,".png"), width=800, height=400)
par(mar=c(2,4,1,1))
...
for(c in sig.chrs){
    png(paste0("~/selection/analysis/",version,"/gscan/mh_plot", results.tag ,".chr", c, ".png"), width=250, height=250)
...
png(paste0("~/selection/analysis/",version,"/gscan/qq_plot_all", results.tag, ".png"), width=400, height=400)
par(mar=c(4,4,1,1))
...
png(paste0("~/selection/analysis/",version,"/gscan/qq_plot_cat", results.tag, ".png"), width=400, height=400)
