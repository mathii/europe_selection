...
xlim=c(-8,8)
xticks <- c(-8, -6, -4, -2, 0, 2, 4, 6, 8)
if(what=="place"){
    xlim=c(-6,6)
    xticks <- c( -6, -4, -2, 0, 2, 4, 6)
}

pdf(paste0("~/selection/analysis/poly/results/", what ,".pdf"), height=5, width=6)
par(mar=c(4.1,4.1,4.1,2.1))
plot(0,0, col="white", bty="n", xaxt="n", yaxt="n", bty="n", xlim=xlim+c(0,2), ylim=c(-0.5, length(traits)+1), xlab="", ylab="")

i=length(traits)
for(trait in traits){
    results <- read.table(paste0("~/selection/analysis/poly/", trait, "/", trait, "_", what, ".results.txt"), as.is=T, header=T)
    
    P1 <- sapply(strsplit(results[,1],","), "[[", 1)
    P2 <- sapply(strsplit(results[,1],","), "[[", 2)
    pairs.full=paste0(P1, "-", P2)
...
#    Arrows(xlim[1], i, xlim[2], i, code=3)
    ...
    points(Z, i+0*Z, pch=16, col=thecols)
    ...
    text(Z[zz], i-0.02, lab[zz], pos=ifelse(offset>0,1,3), cex=0.6)

    text(xlim[2]+0.2, i, trait, pos=4)
    i=i-1
}
